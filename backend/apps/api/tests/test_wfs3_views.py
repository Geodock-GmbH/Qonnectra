...
class TestWFS3ProxyView:
    ...
    def test_proxy_forwards_landing_page(self, api_client, qgis_project):
        ...
        # Verify MAP parameter was added to upstream request
    ...
    def test_proxy_rewrites_urls_in_response(self, api_client, qgis_project):
        ...
        assert response.status_code == 200
        # Check that URLs were rewritten to proxy URLs
...
class TestQGISProjectModel:
    ...
    def test_get_wfs3_url(self, qgis_project):
        """Should return correct WFS3 URL."""
        url = qgis_project.get_wfs3_url()
        assert url == f"/api/v1/wfs3/{qgis_project.name}/"

    def test_map_path_property(self, qgis_project):
        """Should return correct MAP path."""
        map_path = qgis_project.map_path
        assert map_path == f"/projects/{qgis_project.name}.qgz"
