...
describe('dashboard +page.server.js', () => {
	...
	function createLoadArgs(projectId?: string) {
		return {
			fetch: mockFetch,
			cookies: mockCookies,
	...
	describe('no project selected', () => {
		test('should return empty defaults when no projectId', async () => {
			const result = (await load(createLoadArgs())) as Record<string, unknown>;
	...
	describe('successful data loading', () => {
		test('should return mapped statistics on success', async () => {
			...
			const result = (await load(createLoadArgs('1'))) as Record<string, unknown>;
	...
	describe('API call parameters', () => {
		test('should fetch with correct project ID', async () => {
			...
			const statsCall = mockFetch.mock.calls.find((c: unknown[]) =>
				(c[0] as string).includes('dashboard/statistics')
			);
			expect((statsCall as unknown[])[0]).toContain('project=42');
		});
		...
		test('should pass auth headers and credentials', async () => {
			...
			mockFetch.mock.calls.forEach((call: unknown[]) => {
				expect((call[1] as Record<string, unknown>).headers).toEqual(
					expect.objectContaining({ Cookie: 'api-access-token=mock-token' })
				);
				expect((call[1] as Record<string, unknown>).credentials).toBe('include');
			});
		});
	});
