...
export const actions = {
	...
	savePolygon: async ({ request, fetch, cookies, params }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to save polygon' });
		}
	},
	...
	updatePolygon: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to update polygon' });
		}
	},
	...
	renamePolygon: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to rename polygon' });
		}
	},
	...
	deletePolygon: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to delete polygon' });
		}
	}
} satisfies Actions;
