...
export const load: PageServerLoad = async ({ fetch, cookies, params }) => {
	...
	try {
		...
		const statusDevelopments = statusDevelopmentsData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.status
		}));

		const flags = flagsData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.flag
		}));

		const residentialUnitTypes = residentialUnitTypesData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.residential_unit_type
		}));

		const residentialUnitStatuses = residentialUnitStatusesData.map(
		...
		let linkedNodes: Record<string, unknown>[] = [];
		...
		try {
			...
			if (nodesResponse.ok) {
				...
				linkedNodes = features.map((f: Record<string, unknown>) => ({
					uuid: f.id || (f.properties as Record<string, unknown>)?.uuid,
		...
		let residentialUnits: unknown[] = [];
		let linkedTrenchGeometries: unknown[] = [];
		try {
...
export const actions = {
	...
	updateAddress: async ({ request, fetch, cookies, params }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to update address' });
		}
	},
	...
	regenerateId: async ({ fetch, cookies, params }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to regenerate address ID' });
		}
	},
	...
	deleteAddress: async ({ request, fetch, cookies, params }) => {
		...
		try {
			...
			if ((err as Record<string, unknown>).status === 303) throw err;
			console.error('Error deleting address:', err);
			return fail(500, { message: (err as Error).message || 'Failed to delete address' });
		}
	},
	...
	createResidentialUnit: async ({ request, fetch, cookies, params }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to create residential unit' });
		}
	},
	...
	updateResidentialUnit: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to update residential unit' });
		}
	},
	...
	deleteResidentialUnit: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to delete residential unit' });
		}
	}
} satisfies Actions;
