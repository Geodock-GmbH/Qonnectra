...
export const load: PageServerLoad = async ({ fetch, cookies, params }) => {
	...
	try {
		...
		const residentialUnitTypes = typesData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.residential_unit_type
		}));

		const residentialUnitStatuses = statusesData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.status
		}));
...
export const actions = {
	...
	regenerateId: async ({ fetch, cookies, params }) => {
		...
		try {
			...
			return fail(500, {
				message: (err as Error).message || 'Failed to regenerate residential unit ID'
			});
		}
	}
} satisfies Actions;
