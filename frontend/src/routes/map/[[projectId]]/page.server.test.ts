...
describe('map +page.server.js', () => {
	 ...
	 * @param formFields - Key-value pairs to populate the FormData
	 */
	function createEvent(formFields: Record<string, string> = {}) {
	...
	describe('getTrenchProfile', () => {
		test('should call getTrenchProfile with trench UUID', async () => {
			...
			vi.mocked(getTrenchProfile).mockResolvedValueOnce({
				conduits: [{ uuid: 'c-1' }]
			});
