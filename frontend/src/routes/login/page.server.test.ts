		...
		parse: vi.fn((response) => {
			...
			return header.split(',').map((part: string) => {
				...
				attrs.forEach((attr: string) => {
					...
					if (k === 'path') (cookie as Record<string, unknown>).path = val;
					if (k === 'httponly') (cookie as Record<string, unknown>).httpOnly = true;
					if (k === 'secure') (cookie as Record<string, unknown>).secure = true;
					if (k === 'samesite') (cookie as Record<string, unknown>).sameSite = val;
				});
				return cookie;
			});
		})
...
describe('login +page.server.js', () => {
	...
	let mockUrl: Record<string, unknown>;
