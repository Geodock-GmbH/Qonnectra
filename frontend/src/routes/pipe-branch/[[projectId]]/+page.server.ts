...
export const actions = {
	...
	deleteConnection: async ({ request, cookies }) => {
		try {
			const formData = await request.formData();
			const connectionUuid = formData.get('uuid') as string;
	...
	getTrenchesNearNode: async ({ request, cookies }) => {
		try {
			const formData = await request.formData();
			const nodeName = formData.get('node_name') as string;
	...
	getTrenchSelections: async ({ request, cookies }) => {
		try {
			const formData = await request.formData();
			const nodeUuid = formData.get('node_uuid') as string;
	...
	saveTrenchSelections: async ({ request, cookies }) => {
		try {
			const formData = await request.formData();
			const nodeUuid = formData.get('node_uuid') as string;
			const trenchUuidsStr = formData.get('trench_uuids') as string;
