				...
				{:else if result.trace_trees}
					{#each result.trace_trees as tree, i (tree.fiber?.id ?? i)}
						<details class="mb-4" open={i === 0}>
							<summary
								class="flex cursor-pointer items-center gap-3 rounded-lg bg-surface-100-900 px-4 py-3 hover:bg-surface-200-800"
							>
								<span class="h-2 w-2 rounded-full bg-primary-500"></span>
								<span class="font-medium text-surface-900-100">{m.trace_fiber_path({ number: i + 1 })}</span>
							</summary>
							<div class="mt-4 pl-6">
...
{#snippet statCard(label, value, colorClass)}
	<div class="flex flex-col items-center rounded-lg border border-surface-200-800 p-4">
		...
		<span class="mt-1 text-xs font-medium uppercase tracking-wide text-surface-600-400">{label}</span>
	</div>
{/snippet}
...
{#snippet statBadge(label, value)}
	<div class="flex flex-col items-center rounded-lg border border-surface-200-800 p-4">
		<span class="font-mono text-2xl font-bold {value ? 'text-success-500' : 'text-surface-600-400'}">
			{value ? 'Yes' : 'No'}
		</span>
		<span class="mt-1 text-xs font-medium uppercase tracking-wide text-surface-600-400">{label}</span>
	</div>
...
{#snippet fiberDetails(fiber)}
	<div class="flex flex-wrap gap-2 pb-2 pl-1 text-xs">
		...
		{#if fiber.fiber_number_in_bundle}
			<span class="text-surface-600-400"
				>{m.trace_in_bundle()}: <code class="text-surface-700-300">{fiber.fiber_number_in_bundle}</code></span
			>
		{/if}
	...
	<div class="mb-2 ml-1 rounded-lg border-l-2 border-error-500 bg-surface-100-900 p-3 text-sm">
		...
		<div class="flex flex-wrap gap-2 text-xs">
			{#if address.id_address}
				<span class="text-surface-600-400"
					><span class="text-surface-600-400">{m.form_id_address()}:</span> {address.id_address}</span
				>
			{/if}
			{#if address.district}
				<span class="text-surface-600-400"
			...
			{#if address.status_development}
				<span class="text-surface-600-400"
					><span class="text-surface-600-400">{m.form_status()}:</span> {address.status_development}</span
				>
			{/if}
			{#if address.project}
				<span class="text-surface-600-400"
					><span class="text-surface-600-400">{m.form_project({ count: 1 })}:</span> {address.project}</span
				>
			{/if}
			{#if address.flag}
				<span class="text-surface-600-400"
	...
	<div class="mb-2 ml-1 rounded-lg border-l-2 border-tertiary-500 bg-surface-100-900 p-3 text-sm">
		...
		<div class="flex flex-wrap gap-2 text-xs">
			...
			{#if ru.building_section}
				<span class="text-surface-600-400"
					><span class="text-surface-600-400">{m.form_building_section()}:</span> {ru.building_section}</span
				>
			{/if}
			{#if ru.type}
				<span class="text-surface-600-400"
					><span class="text-surface-600-400">{m.form_residential_unit_type()}:</span> {ru.type}</span
				>
			{/if}
			{#if ru.status}
				<span class="text-surface-600-400"
