...
describe('trace/residential-unit/[uuid] +page.server.js', () => {
	/** @type {any} */
	let mockFetch;
	/** @type {any} */
	let mockCookies;

	const TEST_UUID = 'ru-uuid-654';
	...
	function createLoadEvent(searchParams = {}) {
		const url = new URL(`http://localhost/trace/residential-unit/${TEST_UUID}`);
	...
	test('should call fiber-trace API with residential_unit_id', async () => {
		...
		expect(result.result).toEqual({ units: [] });
