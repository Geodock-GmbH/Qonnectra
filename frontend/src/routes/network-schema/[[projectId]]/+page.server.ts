...
export const actions = {
	...
	updateNode: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to update node' });
		}
	},
	deleteNode: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			return fail(500, { message: (err as Error).message || 'Failed to delete node' });
		}
	},
	getNodeDependencies: async ({ request, fetch, cookies, params }) => {
		...
		try {
			...
			let cables: Record<string, unknown>[] = [];
			...
			let structures: Record<string, unknown>[] = [];
			if (structuresResponse.ok) {
				structures = await structuresResponse.json();
			}

			let children: Record<string, unknown>[] = [];
			let childrenWithCables: Record<string, unknown>[] = [];
			if (projectId) {
				...
				if (childrenResponse.ok) {
					...
					for (const child of children) {
						const childId =
							child.id || (child.properties as Record<string, unknown>)?.uuid || child.uuid;
						const childName = (child.properties as Record<string, unknown>)?.name || child.name;
	...
	updateCableConnection: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			return fail(500, {
				message: (err as Error).message || 'Failed to update cable connection'
			});
		}
	},
	...
	deleteCableSplicesAtNode: async ({ request, fetch, cookies }) => {
		...
		try {
			...
			const splicesToDelete = Array.from(spliceMap.values()) as Record<string, unknown>[];
