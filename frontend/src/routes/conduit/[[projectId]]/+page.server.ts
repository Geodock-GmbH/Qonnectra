 ...
 */
export const load: PageServerLoad = async ({ fetch, url, depends, cookies, params }) => {
	...
	try {
		...
		const pipes = (pipesData.results || []).map((item: Record<string, unknown>) => ({
			value: item.uuid,
			name: item.name || '',
		...
		const conduitTypes = conduitTypesData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.conduit_type
		}));

		const statuses = statusesData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.status
		}));

		const networkLevels = networkLevelsData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.network_level
		}));

		const companies = companiesData.map((item: Record<string, unknown>) => ({
			value: item.id,
			label: item.company
		}));
