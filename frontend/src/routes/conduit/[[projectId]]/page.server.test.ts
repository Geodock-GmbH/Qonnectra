...
describe('+page.server.js', () => {
	...
	describe('load function', () => {
		...
		test('should fetch conduits successfully with projectId', async () => {
			...
			expect((result.pipes as Record<string, unknown>[])[0].name).toBe('Conduit 1');
	...
	describe('uploadConduits action', () => {
		...
		test('should return error when no file is uploaded', async () => {
			...
			expect((result.data as Record<string, unknown>).message).toBe(
		...
		test('should handle API error with warnings', async () => {
			...
			expect(result.status).toBe(400);
			expect((result.data as Record<string, unknown>).uploadError).toBe(true);
			expect((result.data as Record<string, unknown>).message).toBe('Some rows failed');
			expect((result.data as Record<string, unknown>).errors).toContain('Row 1: Invalid data');
			expect((result.data as Record<string, unknown>).warnings).toContain('Row 2: Duplicate entry');
		});
