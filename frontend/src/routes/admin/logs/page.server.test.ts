...
describe('admin logs +page.server.js', () => {
	...
	function setupMocks({
		...
		projectsResponse = [] as Record<string, unknown>[],
		logsOk = true,
		projectsOk = true
	} = {}) {
	...
	describe('successful data loading', () => {
		test('should return logs and projects on success', async () => {
			...
			expect((result.logs as Record<string, unknown>[])[0].level).toBe('ERROR');
			expect((result.logs as Record<string, unknown>[])[0].message).toBe('Something failed');
