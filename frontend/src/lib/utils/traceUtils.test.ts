...
import { buildGeoJSON, hasGeometries, traceEntryPath, traceEntryTypeFromSlug } from './traceUtils';
 ...
 */
function makeTraceTree(overrides: Record<string, unknown> = {}): Record<string, unknown> {
	return {
		fiber: {
			id: 'fiber-1',
			cable_name: 'Cable-1',
...
describe('buildGeoJSON', () => {
	...
	it('extracts node Point features from trace_tree', () => {
		...
		const nodeFeatures = geojson.features.filter(
			(f: GeoJSONFeature) => f.properties.feature_type === 'node'
		);
	...
	it('extracts address Point features from trace_tree', () => {
		...
		const addrFeatures = geojson.features.filter(
			(f: GeoJSONFeature) => f.properties.feature_type === 'address'
		);
...
describe('hasGeometries', () => {
	...
	it('returns false for null/undefined result', () => {
		expect(hasGeometries(null as unknown as Parameters<typeof hasGeometries>[0])).toBe(false);
		expect(hasGeometries(undefined as unknown as Parameters<typeof hasGeometries>[0])).toBe(false);
	});
});

describe('traceEntryPath', () => {
	it('should address an entity under its type', () => {
		expect(traceEntryPath('cable', 'c-1')).toBe('/trace/cable/c-1');
	});

	it('should spell the residential unit type with a hyphen', () => {
		expect(traceEntryPath('residential_unit', 'ru-1')).toBe('/trace/residential-unit/ru-1');
	});
});

describe('traceEntryTypeFromSlug', () => {
	it('should read every entry type back from its slug', () => {
		expect(traceEntryTypeFromSlug('fiber')).toBe('fiber');
		expect(traceEntryTypeFromSlug('residential-unit')).toBe('residential_unit');
	});

	it('should reject anything else', () => {
		expect(traceEntryTypeFromSlug('residential_unit')).toBeNull();
		expect(traceEntryTypeFromSlug('trench')).toBeNull();
	});
});
