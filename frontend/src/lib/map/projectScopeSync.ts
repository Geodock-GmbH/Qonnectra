...
type ScopedMapState = Pick<
	MapState,
	'olMap' | 'selectedProject' | 'reinitializeForProject' | 'reinitializeForGlobalView'
>;

/**
 * Keeps a map's tile sources in step with the selected project and the global
 * view toggle. Does nothing until the map is ready, since the sources of an
 * unmounted map are already built for the current scope.
